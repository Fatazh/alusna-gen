...
import { brandKitToTailwindConfig, brandKitToW3cTokens, parseBrandKitImport } from "./brandKitInterop";

const exportedBrandKit = JSON.stringify({
  brandName: "Acme", tagline: "Build better", tone: "modern",
  colors: { primary: "#112233", secondary: "#223344", accent: "#334455", background: "#FFFFFF", text: "#111111" },
...
describe("brand kit interoperability", () => {
  it("imports the existing JSON Brand Kit format", () => {
    expect(parseBrandKitImport(exportedBrandKit)).toMatchObject({
      brandName: "Acme", tone: "modern", primaryColor: { r: 17, g: 34, b: 51 }, textColor: { r: 17, g: 17, b: 17 },
    });
  });

  it("rejects malformed JSON and invalid color values", () => {
    expect(() => parseBrandKitImport("not-json")).toThrow("File bukan JSON");
    expect(() => parseBrandKitImport(exportedBrandKit.replace("#112233", "not-a-color"))).toThrow("Warna primary");
  });
