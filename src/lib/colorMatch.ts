...
function smartComplement(hsl: HSL): RGB {
  ...
  let compL: number;
  if (hsl.l < 30) compL = Math.max(hsl.l + 35, 52);        // dark → lighten comp
  else if (hsl.l > 70) compL = Math.min(hsl.l - 35, 48);   // light → darken comp
  else compL = hsl.l;
...
export function getSmartPairings(primary: RGB): ColorPair[] {
  ...
  const textRgb = bestTextOn(primary) === "#FFFFFF"
    ? grey(96)
    : grey(8);
  ...
  if (isAchromatic) {
    if (isDark) {
      // ── Dark gray / black ─────────────────────────────────────────────────
      pairs.push({ rank: 1, role: "dominan",   rgb: grey(96),            description: "Putih — kontras maksimal"         });
      pairs.push({ rank: 2, role: "pendukung", rgb: grey(78),            description: "Abu-abu terang"                   });
      pairs.push({ rank: 3, role: "pendukung", rgb: tinted(210,18,68),   description: "Abu-abu kebiruan"                 });
      pairs.push({ rank: 4, role: "aksen",     rgb: tinted(210,72,58),   description: "Biru — aksen kuat"               });
      pairs.push({ rank: 5, role: "cadangan",  rgb: grey(28),            description: "Abu-abu gelap — layer sekunder"  });
      pairs.push({ rank: 6, role: "netral",    rgb: tinted(30,12,22),    description: "Hitam hangat — background dalam" });
    } else if (isLight) {
      // ── Light gray / white ────────────────────────────────────────────────
      pairs.push({ rank: 1, role: "dominan",   rgb: grey(8),             description: "Hitam elegan — kontras maksimal" });
      pairs.push({ rank: 2, role: "pendukung", rgb: grey(28),            description: "Abu-abu gelap"                   });
      pairs.push({ rank: 3, role: "pendukung", rgb: tinted(215,18,42),   description: "Abu-abu kebiruan"                });
      pairs.push({ rank: 4, role: "aksen",     rgb: tinted(215,65,45),   description: "Biru — aksen bersih"             });
      pairs.push({ rank: 5, role: "cadangan",  rgb: grey(72),            description: "Abu-abu sedang"                  });
      pairs.push({ rank: 6, role: "netral",    rgb: tinted(30,10,92),    description: "Putih hangat — surface lembut"   });
    } else {
      // ── Mid-tone gray ─────────────────────────────────────────────────────
      const hi = clampL(l < 50 ? l + 55 : l - 55);
      pairs.push({ rank: 1, role: "dominan",   rgb: grey(hi),            description: "Kontras tinggi"                  });
      pairs.push({ rank: 2, role: "pendukung", rgb: grey(clampL(l-30)),  description: "Lebih gelap"                     });
      pairs.push({ rank: 3, role: "pendukung", rgb: grey(clampL(l+30)),  description: "Lebih terang"                    });
      pairs.push({ rank: 4, role: "aksen",     rgb: tinted(30,45,62),    description: "Sentuhan hangat"                 });
      pairs.push({ rank: 5, role: "cadangan",  rgb: tinted(210,30,55),   description: "Sentuhan dingin"                 });
      pairs.push({ rank: 6, role: "netral",    rgb: tinted(30,6,l),      description: "Versi sedikit hangat"            });
    }
  } else {
    // ── Chromatic color ───────────────────────────────────────────────────
    const { h, s } = hsl;
    ...
    pairs.push({ rank: 1, role: "dominan",   rgb: dominant, description: "Komplementer 180° — pasangan paling kontras" });
    pairs.push({ rank: 2, role: "pendukung", rgb: analog1,  description: "Analogus −30° — harmonis berdekatan"         });
    pairs.push({ rank: 3, role: "pendukung", rgb: analog2,  description: "Analogus +30° — harmonis berdekatan"         });
    pairs.push({ rank: 4, role: "aksen",     rgb: triad,    description: "Triadik 120° — pop/aksen"                    });
    pairs.push({ rank: 5, role: "cadangan",  rgb: split,    description: "Split komplemen 210° — alternatif lembut"    });
    pairs.push({ rank: 6, role: "netral",    rgb: neutral,  description: "Versi netral — cocok untuk background/teks"  });
  }

  // Append text color last
  pairs.push({ rank: 7, role: "teks", rgb: textRgb, description: "Warna teks paling terbaca di atas warna ini" });
