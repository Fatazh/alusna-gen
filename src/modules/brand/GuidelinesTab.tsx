        ...
        <Card key={g.section}>
          ...
          <CardBody>
            <div className="space-y-1">
              {g.items.map((item) => (
                <div key={item.label} className="flex items-center justify-between rounded-lg border px-4 py-2.5 transition" style={{ borderColor: "var(--border)" }}>
                  <div className="flex items-center gap-3">
                    {item.hex && (
                      <div className="h-6 w-6 rounded-md border" style={{ borderColor: "var(--border)", backgroundColor: item.hex }} />
                    )}
