...
export function DesignSystemModule() {
  ...
  const exportedCode = useMemo(
    () => exportDesignSystem(ds, exportFormat),
    [ds, exportFormat],
  );
      ...
      <Card>
        <CardHeader
          ...
          action={
            <div className="flex items-center gap-2">
              <label className="text-xs" style={{ color: "var(--text-muted)" }}>Prefix:</label>
              <input
                type="text"
                value={dsName}
                onChange={(e) =>
                  setDsName(sanitizeTokenName(e.target.value) || "brand")
                }
        ...
        <CardBody>
          ...
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl text-lg font-bold text-white" style={{ backgroundColor: rgbToHex(selectedColor) }}>
            ...
            <div>
              ...
              <div className="text-xs" style={{ color: "var(--text-muted)" }}>
                {ds.colors.length} colors · {ds.shades.length} shades · {ds.typography.length} type styles · {ds.spacing.length} spacing · {ds.radius.length} radii · {ds.shadows.length} shadows
              </div>
            </div>
          </div>
        </CardBody>
        ...
        <div className="space-y-4 xl:col-span-2">
          ...
          >
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
              {ds.colors.map((c) => (
                <div key={c.role} className="group rounded-xl border overflow-hidden transition" style={{ borderColor: "var(--border)" }}>
                  <div className="h-16 w-full" style={{ backgroundColor: c.hex }} />
                  <div className="px-3 py-2">
                    <div className="text-[11px] font-semibold" style={{ color: "var(--text-primary)" }}>{c.role}</div>
                    <div className="font-mono text-[10px]" style={{ color: "var(--text-muted)" }}>{c.hex}</div>
                    <div className="mt-1 text-[10px] leading-tight opacity-0 group-hover:opacity-100 transition" style={{ color: "var(--text-muted)" }}>{c.usage}</div>
                  </div>
                </div>
              ))}
            </div>
              ...
              {ds.shades.map((s) => {
                ...
                return (
                  <div key={s.step} className="group relative flex-1">
                    <div className="h-20 w-full transition-transform group-hover:scale-110 group-hover:z-10" style={{ backgroundColor: hex }} />
                    <div className="mt-1 text-center">
                      <div className="text-[10px] font-medium" style={{ color: "var(--text-secondary)" }}>{s.step}</div>
                      <div className="text-[9px] font-mono" style={{ color: "var(--text-muted)" }}>{hex}</div>
                    </div>
                  </div>
                );
              })}
          ...
          >
            <div className="space-y-2">
              {ds.typography.map((t) => (
                <div key={t.name} className="flex items-baseline gap-4 rounded-lg border px-4 py-3 transition" style={{ borderColor: "var(--border)" }}>
                  <div className="w-24 shrink-0">
                    <div className="text-[11px] font-semibold" style={{ color: "var(--text-secondary)" }}>{t.name}</div>
                    <div className="text-[10px]" style={{ color: "var(--text-muted)" }}>{t.size} / {t.lineHeight}</div>
                  </div>
                  <div
                  ...
                  <div className="text-[10px] hidden sm:block" style={{ color: "var(--text-muted)" }}>{t.usage}</div>
                </div>
              ))}
            </div>
          </CollapsibleSection>
            ...
            <div className="flex items-end gap-2 flex-wrap">
              {ds.spacing.map((s) => (
                <div key={s.name} className="group flex flex-col items-center">
                  ...
                  />
                  <div className="mt-1 text-[10px]" style={{ color: "var(--text-muted)" }}>{s.name}</div>
                  <div className="text-[9px] font-mono" style={{ color: "var(--text-muted)" }}>{s.px}px</div>
                </div>
              ))}
            </div>
          </CollapsibleSection>
            ...
            <div className="flex items-end gap-3 flex-wrap">
              {ds.radius.map((r) => (
                <div key={r.name} className="group flex flex-col items-center">
                  ...
                  />
                  <div className="mt-1 text-[10px]" style={{ color: "var(--text-muted)" }}>{r.name}</div>
                  <div className="text-[9px] font-mono" style={{ color: "var(--text-muted)" }}>{r.value}</div>
                </div>
              ))}
            </div>
          </CollapsibleSection>
            ...
            <div className="flex gap-4 flex-wrap">
              {ds.shadows.map((s) => (
                <div key={s.name} className="flex flex-col items-center">
                  ...
                  />
                  <div className="mt-2 text-[10px]" style={{ color: "var(--text-muted)" }}>{s.name}</div>
                  <div className="text-[9px] max-w-[120px] text-center" style={{ color: "var(--text-muted)" }}>{s.usage}</div>
                </div>
              ))}
            </div>
          </CollapsibleSection>
        ...
        <div className="space-y-4">
          <Card className="sticky top-20">
            <CardHeader
              ...
              subtitle="Copy the generated tokens"
            />
            <CardBody className="space-y-4">
                  ...
                  {EXPORT_FORMATS.map((f) => (
                    <button
                      ...
                      style={exportFormat === f.id
                        ? { backgroundColor: "rgba(99,102,241,0.15)", color: "#6366f1", borderColor: "rgba(99,102,241,0.3)", border: "1px solid rgba(99,102,241,0.3)" }
                        : { borderColor: "var(--border)", color: "var(--text-secondary)", border: "1px solid var(--border)" }
                      }
                    >
                      <span>{f.icon}</span>
                      {f.label}
              ...
              <div className="relative">
                ...
                <pre className="max-h-96 overflow-auto rounded-xl border p-4 text-[11px] leading-relaxed" style={{ borderColor: "var(--border)", backgroundColor: "var(--surface)", color: "var(--text-secondary)" }}>
